...
{	
	...
	public class AssetBundleManager : MonoBehaviour
	{
        ...
        static int _LoadingCnt = 0;

		static Dictionary<string, string> m_DownloadingErrors = new Dictionary<string, string> ();
		static List<AssetBundleLoadOperation> m_InProgressOperations = new List<AssetBundleLoadOperation> ();
        static int _InProgressOpCnt = 0;
		...
		private static void Log(LogType logType, string text)
		{
            if (logType == LogType.Error)
                Debug.LogError("[AssetBundleManager " + Time.time + "] " + text);
            else if (m_LogMode == LogMode.All)
                Debug.Log("[AssetBundleManager " + Time.time + "] " + text);

            //if (logType == LogType.Error)
            //    GuiTextDebug.debug("[AssetBundleManager error]" + text);
            //else if (m_LogMode == LogMode.All)
            //    GuiTextDebug.debug("[AssetBundleManager]" + text);
        }

        static string GetLocalAssetURL(string assetBundleName)
        {
            return m_BaseDownloadingURL + assetBundleName;
        }
        ...
        {
            m_BaseDownloadingURL = GetStreamingAssetsPath();
        }
		...
		{
			return Initialize(Utility.GetPlatformName() + ".common");
		}
		...
		static protected bool LoadAssetBundleInternal (string assetBundleName, bool isLoadingAssetBundleManifest)
		{
			...
			WWW download = null;
			string url = GetLocalAssetURL(assetBundleName);
		...
		void Update()
		{
            ...
            if (_LoadingCnt > 0)
            {
                ...
                foreach (var keyValue in m_DownloadingWWWs)
                {
                    ...
                    if (download.isDone)
                    {
                        //GuiTextDebug.debug("download.isDone " + (download.url));
                        Debug.Log("download.isDone " + (download.url));
                        byte[] bytes = download.bytes;
                        //AssetBundle bundle = download.assetBundle;
                        BundleEncryption.Decryption(bytes);
                ...
                {
                    WWW download = m_DownloadingWWWs[keysToRemove[i]];
        ...
        public static int GetAssetCntInBundle(string bundleName)
        {
#if UNITY_EDITOR
            if (SimulateAssetBundleInEditor)
            {
                ...
                if (assetPaths.Length == 0)
                {
                    Debug.LogError("There is no asset with name \"" + bundleName);
                    return 0;
                }
...
#endif
            string errorStr = "";
            LoadedAssetBundle bundle = AssetBundleManager.GetLoadedAssetBundle(bundleName.ToLower(), out errorStr);
            if (bundle == null)
            {
                Debug.Log(errorStr);
                return 0;
            ...
            return bundle.m_AssetBundle.GetAllAssetNames().Length;
        }

        public static Object LoadAssetSyn(string bundleName, string assetName, System.Type assetType)
        {
#if UNITY_EDITOR
            if (SimulateAssetBundleInEditor)
            {
                ...
                // @TODO: Now we only get the main object from the first asset. Should consider type also.
                Object target = AssetDatabase.LoadAssetAtPath(assetPaths[0], assetType);
                return target;
            }
#endif
            string errorStr = "";
            ...
            return bundle.m_AssetBundle.LoadAsset(assetName, assetType);
        }
        ...
        public static bool IsBundleManagerInitFinish()
        {
            if (m_AssetBundleManifest == null)
                return false;
            else
