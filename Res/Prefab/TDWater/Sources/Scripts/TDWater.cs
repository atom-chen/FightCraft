...
[ExecuteInEditMode]
public class TDWater : MonoBehaviour
{
    //void Update()
    //{
    //    if( !renderer )
    ...
    //    WaterTileBeingRendered(Camera.main);
    ...
    {
        WaterTileBeingRendered(Camera.current);
    }

    public void WaterTileBeingRendered(Camera current)
    {
        if (current != null)
        {
